...
metadata <- read.table("~/Desktop/Fragmentomics_GOOD/DELFI/metadata_discovery.txt") #Read in metadata table
...
ggplot(df.fr3, aes(x=bin, y=ratio.centered, group=id)) +
  geom_line(color="gray50", size=1) +
  #stat_summary(aes(group = type), geom = "line", fun.y = median, size = 1.5, color="steelblue")
...
#save_plot("./tmp.pdf", gg, ncol=1, nrow=1, base_height=12, base_width=35)
...
ggplot(df.fr3, aes(x=bin, y=ratio.centered, group=id)) +
  geom_line(color="gray50", size=1) +
  labs(x="", y="Fragmentation profile", color="") +
  facet_grid(DIAGNOSIS_CLASS~arm, 
             switch="x",
             space="free_x", 
             scales="free_x", 
...
control.summary.tib = summary.df.2 %>% filter(DIAGNOSIS_CLASS=="CONTROL") %>% select(ratio2.corrected.cor)
